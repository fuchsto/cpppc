...
/* -----------------------------------------------------------------------
 * Slightly modified version of the implementation submitted by Felix.
 * -----------------------------------------------------------------------
 */
...
class sparse_array {
 ...
 public:
  ...
  typedef element_t                                   value_t;

  friend iterator;
  friend const_iterator;
  friend proxy_reference;

 public:  // Should be private but friend is not working properly
 ...
 public:
  ...
  proxy_reference at(const size_t pos) {
    if (!(pos < N)) {
      throw std::out_of_range("index out of range");
    }
    return proxy_reference(*this, pos);
  }
  ...
  proxy_reference operator[](const size_t pos) {
    return proxy_reference(*this, pos);
  }

  proxy_reference front() { return at(0); }
  const T &       front() const { return at(0); }
  proxy_reference  back() { return at(N - 1); }
